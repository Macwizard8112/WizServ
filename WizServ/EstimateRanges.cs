...
namespace WizServ
{
    public partial class EstimateRanges : Form
    {
        public Icon image100 = Properties.Resources.WizServ;
        private int loopCount;
        private int digitalCountW, datCountW, digitalCountNW, datCountNW;
        public decimal datSumW, digitalSumW, datSumNW, digitalSumNW, datSumWT, datSumNWT, digitalSUMWT, digitalSumNWT;
        private readonly string avlhist = @"I:\\Datafile\\Control\\Avlhist.CSV";

        public EstimateRanges()
        {
            InitializeComponent();
            Icon = image100;
            MaximizeBox = false;
        ...
        private void PullNumbers()
        {
            try
            {
                StreamReader reader = new StreamReader(avlhist, Encoding.GetEncoding("Windows-1252"));
                ...
                while (!reader.EndOfStream)
                {
                    ...
                    if (listBJ[loopCount] == "POWERED SPEAKRS")
                    {
                        
                        if (listAT[loopCount] == "NON-WARRANTY")
                        {
                            datCountNW++;
                            decimal d = decimal.Parse(listR[loopCount]) + decimal.Parse(listS[loopCount]);
                            datSumNW = datSumNW + d;
                        }
                        if (listAT[loopCount] == "WARRANTY XX")
                        {
                            datCountW++;
                            decimal d = decimal.Parse(listR[loopCount]) + decimal.Parse(listS[loopCount]);
                            datSumW = datSumW + d;
                    ...
                    if (listBJ[loopCount] == "POWER AMPLIFIER")
                    {
                        if (listAT[loopCount] == "NON-WARRANTY")
                        {
                            digitalCountNW++;
                            decimal d = decimal.Parse(listR[loopCount]) + decimal.Parse(listS[loopCount]);
                            digitalSumNW = digitalSumNW + d;
                        }
                        if (listAT[loopCount] == "WARRANTY XX")
                        {
                            digitalCountW++;
                            decimal d = decimal.Parse(listR[loopCount]) + decimal.Parse(listS[loopCount]);
                            digitalSumW = digitalSumW + d;
                        }
                    }
                    loopCount++;
                }
                ...
                try
                {
                    if (datSumNW == 0 | datCountNW == 0)
                    {
                        datSumNWT = 0;
                    }
                    else
                    {
                        datSumNWT = datSumNW / datCountNW;
                    }
                    if (datSumW == 0 | datCountW == 0)
                    {
                        datSumWT = 0;
                    }
                    else
                    {
                        datSumWT = datSumW / datCountW;
                    }
                    label3.Text = datSumNW.ToString("C2");
                    label4.Text = datSumW.ToString("C2");
                    label7.Text = datCountNW.ToString();
                    label8.Text = datCountW.ToString();
                    label14.Text = datSumNWT.ToString("C2");
                    label16.Text = datSumWT.ToString("C2");
                }
                catch (Exception)
                {
                    MessageBox.Show("Error 215-222");
                }
            }
            catch (Exception ex)
            {
                MessageBox.Show("Error 252: Sorry an error has occured: " + ex.Message);
            }
        }
    }
}
