...
namespace WizServ
{
    public partial class ByCityName : Form
    {
        ...
        public string SearchTerm, mLastFirst, mCityState;
        public string mLN, mCS;
        ...
        {
            SearchTerm = listBox1.SelectedItem.ToString();  // Select whole line
            SearchTerm = SearchTerm.Substring(0, 6);        // Pull Claim #
            Version.Claim = SearchTerm.Trim();
            claim_no = SearchTerm.Trim();
        ...
        public void GetData()
        {
            try
            {
                ...
                while (!reader.EndOfStream)
                {
                    ...
                    mLN = listE[loopCount] + ", " + listD[loopCount];   // Last, First Name
                    mCS = listG[loopCount];                             // City

                    SetNames();



                    if (listG[loopCount].Contains(claim_no))
                    {
                        listBox1.Items.Add(listB[loopCount]  + " " + mLN + " " + mCS + " " + listH[loopCount] + "\t\t" + listBB[loopCount]);
                        loop++;
                    }
                    loopCount++;
                }
            ...
            {
                MessageBox.Show("Error 258: Sorry an error has occured: " + ex.Message);
            }
        }
    }
}
