...
namespace WizServ
{
    public partial class ByClientDealer2 : Form
    {
        ...
        public void GetData()
        {
            try
            {
                ...
                while (!reader.EndOfStream)
                {
                    ...
                    var Dealer_Num = listBO[loopCount];
                    var mModel = listD[loopCount];
                    if (mModel.Length <= 16)
                    {
                        mModel += "\t";
                    }
                    var mSO = listE[loopCount] + ", " + mModel;
                    if (mSO.Length <= 18)
                    {
                        mSO += "\t\t";
                    }
                    ...
                    if (listBO[loopCount] == claim_no)
                    {
                        if (listG[loopCount].Length <= 13)
                        {
                            listG[loopCount] += "\t";
                        }
                        if (mSO.Length <= 15)
                        ...
                        if (mSO.Length != 31)
                        {
                            richTextBox1.Text = richTextBox1.Text + listB[loopCount] + "\t" + mSO + "\t\t" + listG[loopCount] + "\t" + listH[loopCount] + "\t" + listAN[loopCount] + "\n";
                        }
                        if (mSO.Length == 31)
                        {
                            if (mSO == "GUITAR CENTER #723, AKAI MPK88\t")
                            {
                                richTextBox1.Text = richTextBox1.Text + listB[loopCount] + "\t" + mSO + "\t\t" + listG[loopCount] + "\t" + listH[loopCount] + "\t" + listAN[loopCount] + "\n";
