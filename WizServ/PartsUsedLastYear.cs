...
namespace WizServ
{
    public partial class PartsUsedLastYear : Form
    {
        public Icon image100 = Properties.Resources.WizServ;
        private readonly string PrimaryParts = @"I:\\datafile\\Control\\part_pri.csv";
        private int loopCount, loop, items, onorder, printcount, year, yearnow2;
        private decimal cost;

        public PartsUsedLastYear()
        {
            InitializeComponent();
            Icon = image100;
            MaximizeBox = false;
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            TextWriter txt = new StreamWriter("I:\\Datafile\\Doc\\PriPartsLastYear.txt");
            txt.Write(richTextBox1.Text);
            txt.Close();
            var fileToOpen = "I:\\Datafile\\Doc\\PriPartsLastYear.txt";
        ...
        public void GetData()
        {
            try
            {
                ...
                while (!reader.EndOfStream)
                {
                    ...
                    try
                    {
                        var ord_date = Convert.ToDateTime(listAN[loopCount]);
                        year = ord_date.Year;
                        var yearnow = DateTime.Now.ToShortDateString();
                        var yearnow1 = Convert.ToDateTime(DateTime.Now);
                        yearnow2 = yearnow1.Year;
                        yearnow2--; // Subtract 1 from current year to show last year.
                    }
                    catch (Exception ex)
                    {
                        MessageBox.Show("There is a bad date in datebase: datafile\\Control\\part_pri.csv");
                    }
                    if (yearnow2 == yearnow2)
                    {
                        if (listK[loopCount] != "0")
                        {
                ...
                label9.Text = "Total Cost: " + cost.ToString("C2");
            }
            catch (Exception ex)
            {
                MessageBox.Show("Error 256: Sorry an error has occured: " + ex.Message);
            }
        }
    }
}
